...
export default function SystemCard({ system, index }) {
  ...
  return (
    <div
      className="system-card group relative bg-dark-bg-3/40 border border-white/8 rounded-2xl p-6 lg:p-8 backdrop-blur-sm overflow-hidden cursor-pointer transition-transform duration-300 hover:-translate-y-1"
    ...
    >
      ...
      <p className="font-mono text-sm text-white/40 mb-4">{system.subtitle}</p>
      ...
      {/* See More button */}
      <Link href={`/systems/${system.slug}`} legacyBehavior>
        <a
